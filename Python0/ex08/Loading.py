...
def ft_tqdm(lst: range) -> None:
	...
	for i, item in enumerate(lst):
		...
		formatted_time = f"{int(elapsed_time // 60):02d}:{int(elapsed_time % 60):02d}"
		formatted_eta = f"{int(eta // 60):02d}:{int(eta % 60):02d}"

		bar = f"|{'█' * progress:<{bar_size}}|"
		percent = progress * 100 // bar_size

		print(f"{percent}%{bar} {step}/{totlen} [{formatted_time}<{formatted_eta} {speed:.2f}it/s]", end="\r", flush=True)
		yield item
